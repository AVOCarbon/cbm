...
/****** Object:  StoredProcedure [dbo].[insert_BI_AVOCARBON_tst]    Script Date: 31/05/2018 11:25:31 ******/
SET ANSI_NULLS ON
GO
...
-- 31/05/18 : this procedure is schedule by SQL agent for AVOCarbon France
...
-- Requete FI_D6_Inventory
	...
	FROM 
		[AVOCARBON].[dbo].[DISPO]
			...
			LEFT JOIN [AVOCARBON].[dbo].[ARTICLE] ON GQ_ARTICLE = GA_ARTICLE --adding the left join!
...
-- Requete FI_D7_Sales
	...
	FROM 
		[AVOCARBON].[dbo].[LIGNE]
			INNER JOIN [AVOCARBON].[dbo].[PIECE] -- change from left join to inner join
				ON
	...
	WHERE 
		...
		AND [AVOCARBON].[dbo].[LIGNE].GL_CODEARTICLE != ''
